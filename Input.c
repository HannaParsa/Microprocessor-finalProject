...
void wave_switch_init(void){
	...
	GPIOB->MODER &= ~GPIO_MODER_MODE12_Msk; // set pb12 as input
	
	
	// pull-down confg for 5,6,7,8,9,10,12,13
	...
	// set EXTI sources for gpio pins
	
	SYSCFG->EXTICR[1] |= SYSCFG_EXTICR2_EXTI5_PB; // exti line5 portb pin5
	...
	// set priroty, clear pending IRQs, enable IRQs
...
void params_adc_init(void) {
	...
	// disable pull-up pull-down for pa0, pa1: preventing unintended changes in voltage
	...
	// sets the sequence of channels to be converted. In this case, PA0 is configured as the first conversion and PA1 as the second conversion.
	...
	// enable the scan mode: automatically sample the chanels.
